	...
	public boolean onCommand(CommandSender sender, Command cmd, String cmdLine, String[] split) {
		try {
			...
			if (split[0].equalsIgnoreCase("price")) {
				if (player.hasPermission("setrankpb.iconomy.price")) {
					if (plugin.prices.containsKey(split[1]) && plugin.useiConomy && plugin.useEconomy) {
						...
						if (!player.hasPermission("setrankpb.iconomy.buy.*") || !player.hasPermission("setrankpb.iconomy.buy." + split[1])) {
							player.sendMessage(ChatColor.RED + "You don't have permission to buy this rank.");
							return true;
						}
			...
			if (split[0].equalsIgnoreCase("buy")) {
				if (!player.hasPermission("setrankpb.iconomy.buy." + split[1]) || !player.hasPermission("setrankpb.iconomy.buy.*")) {
					player.sendMessage(plugin.noPermission);
					return true;
				}
				if (player.hasPermission("setrankpb.iconomy.buy." + split[1]) || player.hasPermission("setrankpb.iconomy.buy.*")) {
			...
			if (split[0].equalsIgnoreCase("pricelist")) {
				...
				for (String s : plugin.prices.keySet()) {
					player.sendMessage(ChatColor.GREEN + s + ChatColor.GRAY + ": " + ChatColor.GREEN + plugin.prices.get(s));
				}
				return true;
			}
			...
			player.sendMessage(ChatColor.RED + "/promo pricelist - Shows a list of prices");
			return true;
			...
			sender.sendMessage(ChatColor.RED + "/promo pricelist - Shows a list of prices");
			return true;
		}
	}
}
