	...
	public boolean onCommand(CommandSender sender, Command cmd, String cmdLine,
		...
		try {
			...
			if (split[0].equalsIgnoreCase("price")) {
				if (player.hasPermission("setrankpb.iconomy.price")) {
					if (plugin.prices.containsKey(split[1])
							&& plugin.useiConomy && plugin.useEconomy) {
						player.sendMessage(ChatColor.GREEN + "The rank "
								+ split[1] + ChatColor.GREEN + " costs "
								+ plugin.prices.get(split[1]) + ".");
						if (!player.hasPermission("setrankpb.iconomy.buy.*")
								|| !player
										.hasPermission("setrankpb.iconomy.buy."
												+ split[1])) {
							player.sendMessage(ChatColor.RED
									+ "You don't have permission to buy this rank.");
							return true;
						}
						...
						if (holdings.hasEnough(plugin.prices.get(split[1]))) {
							player.sendMessage(ChatColor.GREEN
									+ "Do you want to buy it? (Say \"Yes\" or \"No\")");
						...
						player.sendMessage(ChatColor.RED
								+ "The iConomy plugin hasn't been set up. Please contact the server admin.");
						return true;
					} else if (!plugin.useEconomy) {
						player.sendMessage(ChatColor.RED
								+ "This server doesn't use economy.");
						return true;
					} else if (!plugin.prices.containsKey(split[1])) {
						player.sendMessage(ChatColor.RED
								+ "That rank doesn't have a price or doesn't exist.");
						return true;
					} else {
			...
			if (split[0].equalsIgnoreCase("buy")) {
				if (!player.hasPermission("setrankpb.iconomy.buy." + split[1])
						|| !player.hasPermission("setrankpb.iconomy.buy.*")) {
					player.sendMessage(plugin.noPermission);
					return true;
				}
				if (player.hasPermission("setrankpb.iconomy.buy." + split[1])
			...
			if (split[0].equalsIgnoreCase("whatami")) {
				if (player.hasPermission("setrankpb.whatami")) {
					player.sendMessage(ChatColor.GREEN + "You are "
							+ plugin.getHandler().getRank(player));
					return true;
				}
			}
			if (split[0].equalsIgnoreCase("pricelist")) {
				if (!player.hasPermission("setrankpb.iconomy.pricelist")) {
					player.sendMessage(plugin.noPermission);
				}
				for (String s : plugin.prices.keySet()) {
					player.sendMessage(ChatColor.GREEN + s + ChatColor.GRAY
							+ ": " + ChatColor.GREEN + plugin.prices.get(s));
				}
				return true;
			}
			player.sendMessage(ChatColor.RED + "Wrong Syntax! Commands:");
			player.sendMessage(ChatColor.RED
					+ "/promo price <rank> - View the price of a rank");
			player.sendMessage(ChatColor.RED + "/promo buy <rank> - Buy a rank");
			player.sendMessage(ChatColor.RED
					+ "/promo whatami - Shows you what rank you are");
			player.sendMessage(ChatColor.RED
					+ "/promo pricelist - Shows a list of prices");
			return true;
