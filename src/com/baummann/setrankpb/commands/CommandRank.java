		...
		try {
			if (sender instanceof Player) {
				...
				if (player.hasPermission("setrankpb.rankall") || player.hasPermission("setrankpb.rank." + split[1])) {
					...
					if (player.hasPermission("setrankpb.norank." + plugin.getHandler().getRank(target)) && !player.hasPermission("setrankpb.rankall")) {
						player.sendMessage(ChatColor.RED + "You may not rank that player!");
						return true;
					}
					if (!target.isOnline()) {
						if (!plugin.rankOffline) {
							player.sendMessage(ChatColor.RED + "That player is offline!");
						}
						handler.setOfflineRank(split[0], split[1]);
						player.sendMessage("You've changed " + split[0] + "'s rank to " + split[1] + ".");
						return true;
					}
					handler.setRank(target, split[1]);
					player.sendMessage("You've changed " + target.getName() + "'s rank to " + split[1] + ".");
					try {
						if (target instanceof SpoutPlayer && plugin.achievements && plugin.useSpout) {
							try {
								((SpoutPlayer) target).sendNotification("SetRankPB", "You are now " + split[1], Material.getMaterial(plugin.icon.toUpperCase().replace(" ", "_")));
							} catch (NullPointerException e) {
								System.out.println("[SetRankPB] WARNING: The material " + plugin.icon.toUpperCase().replace(" ", "_") + " does not exist. Using default.");
								((SpoutPlayer) target).sendNotification("SetRankPB", "You are now " + split[1], Material.DIAMOND_PICKAXE);
							} catch (Exception e) {
							}
						}
				...
				if (!plugin.canUse) {
					...
					sender.sendMessage(ChatColor.RED + "This server is not running PermissionsBukkit which makes the plugin unusable.");
					sender.sendMessage(ChatColor.RED + "Please download SuperpermsBridge and PermissionsBukkit to get the plugin to work.");
					return true;
				}
				if (sender.hasPermission("setrankpb.rankall") || sender.hasPermission("setrankpb.rank." + split[1])) {
					Player target = plugin.getServer().getPlayer(split[0].toLowerCase());
					if (sender.hasPermission("setrankpb.norank." + plugin.getHandler().getRank(target)) && !sender.hasPermission("setrankpb.rankall")) {
						sender.sendMessage(ChatColor.RED + "You may not rank that sender!");
						return true;
					}
					if (!target.isOnline()) {
						if (!plugin.rankOffline) {
							sender.sendMessage(ChatColor.RED + "That sender is offline!");
						}
						handler.setOfflineRank(split[0], split[1]);
						sender.sendMessage("You've changed " + split[0] + "'s rank to " + split[1] + ".");
						return true;
					}
					handler.setRank(target, split[1]);
					sender.sendMessage("You've changed " + target.getName() + "'s rank to " + split[1] + ".");
					try {
						if (target instanceof SpoutPlayer && plugin.achievements && plugin.useSpout) {
							try {
								((SpoutPlayer) target).sendNotification("SetRankPB", "You are now " + split[1], Material.getMaterial(plugin.icon.toUpperCase().replace(" ", "_")));
