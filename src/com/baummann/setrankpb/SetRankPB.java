...
public class SetRankPB extends JavaPlugin {
	...
	private final SRPBPlayerListener playerListener = new SRPBPlayerListener(
			this);
	...
	public void onEnable() {
		...
		if (spout != null) {
			...
			System.out
					.println("[SetRankPB] Couldn't find Spout, not using achievements.");
			achievements = false;
		}
	...
	public void loadConfig() {
		...
		config.setHeader(
				...
				"#Prices must be a decimal number i.e. 1.0 instead of 1");
		prices = (LinkedHashMap<String, Double>) config
				.getProperty("SetRankPB.iConomy.prices");
		if (prices == null) {
		...
		achievements = config.getBoolean("SetRankPB.Spout.use-achievements",
				true);
		icon = config.getString("SetRankPB.Spout.achievement-icon",
				"diamond pickaxe");
		config.save();
	}
